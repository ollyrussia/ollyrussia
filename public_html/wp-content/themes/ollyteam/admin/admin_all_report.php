            ...
            <div class="col-12">
              <div class="card">
                <div class="card-header">
                  ...
                  <h3 class="card-title">Отчеты по окнам</h3>
				</div>
                      ...
                      <tbody>
 						<?
							$users = get_users('role=psychologist&orderby=meta_value&meta_key=last_name' );
								...
								echo "<option value='".$user->ID."'>".$user->user_lastname." ".$user->user_firstname."</otion>";	
								
								$reports = $wpdb->get_results($wpdb->prepare("
								SELECT wp_olly_booking.status,COUNT(status) as `counter`,wp_olly_booking.type
								FROM wp_olly_booking
								WHERE wp_olly_booking.therapist = %d
								GROUP BY wp_olly_booking.status,wp_olly_booking.type,wp_olly_booking.date_event
								ORDER BY wp_olly_booking.date_event",$user->ID));

								foreach( $reports as $report)
								{

									echo '
									<tr><td colspan="3">'.$user->user_lastname." ".$user->user_firstname.'</td></tr>
									<tr>
