...
class YclientsApi
{
    ...
    public function postRecords(
        ...
        if ($services !== null) {
            ...
            //array_push($parameters['services'], $services);
        }
		...
		if ($custom !== null) {
            $parameters['custom_fields'] = $custom;
        }
	...
	/**
     * Просмотреть расписание работы сотрудника
     *
     * @param integer $companyId - ID компании
     * @param integer $staffId
     * @param string $userToken - Токен для авторизации пользователя
     ...
     * @throws YclientsException
     */
    public function getSchedule($companyId, $staffId, $userToken, $fields=array())
    {
		$startDate='20230424';
		$finDate='20230430';
        return $this->request('schedule/' . $companyId . '/' . $staffId . '/' . $startDate . '/' . $finDate, $fields, self::METHOD_GET, $userToken);
    }
     ...
     * @throws YclientsException
     */
    public function getTimetableSeances($companyId, $date, $staffId, $userToken)
    {
        return $this->request('timetable/seances/' . $companyId . '/' . $staffId . '/' . $date, [],
            self::METHOD_GET, $userToken);
    }
    ...
    protected function request($url, $parameters = [], $method = 'GET', $auth = true)
    {
		...
		//array_push($headers, "Accept: application/vnd.yclients.v2+json");
		array_push($headers, "Accept: application/vnd.api.v2+json");
